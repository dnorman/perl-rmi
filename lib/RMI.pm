...
sub setup {
    ...
    $class->setup_class(   delete $param{engine} );
}
...
sub setup_engine {
    ...
    if ( $ENV{MOD_PERL} ) {
	  ...
	  if ( $software eq 'mod_perl' ) {
		if ( !$engine ) {
		      if     ( $version >= 1.99922 ) { $engine = 'RMI::Engine::Apache2'
		      }elsif ( $version >= 1.9901  ) { $engine = 'RMI::Engine::Apache2'
		      }elsif ( $version >= 1.24    ) { $engine = 'RMI::Engine::Apache'
		      }else {
